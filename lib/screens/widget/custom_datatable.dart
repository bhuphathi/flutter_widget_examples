...
class CustomDataTablePage extends StatelessWidget {
  const CustomDataTablePage({Key? key}) : super(key: key);
...
final _fixedRowCells = ["Math", "Informatics", "Geography", "Physics", "Biology"];
...
class CustomDataTableState<T> extends State<CustomDataTable<T>> {
  ...
  Widget _buildFixedCol() => DataTable(
      ...
      columns: [DataColumn(label: _buildChild(widget.fixedColWidth, widget.fixedColCells.first))],
  ...
  Widget _buildSubTable() => Material(
      ...
      child: DataTable(
          ...
          rows: widget.rowsCells
              .map(
                (row) =>
                    DataRow(cells: row.map((c) => DataCell(_buildChild(widget.cellWidth, c))).toList()),
              )
              .toList()));
